...
fn host_program_override_takes_precedence() {
    let override_path = OsString::from("custom-code-mode-host");
    let default_program = PathBuf::from("codex-code-mode-host");

    assert_eq!(
        resolve_host_program(Some(override_path), default_program),
        PathBuf::from("custom-code-mode-host")
    );
}

#[test]
fn host_program_uses_install_context_default_without_override() {
    let default_program = PathBuf::from("codex-code-mode-host");

    assert_eq!(
        resolve_host_program(/*override_path*/ None, default_program.clone()),
        default_program
    );
}
